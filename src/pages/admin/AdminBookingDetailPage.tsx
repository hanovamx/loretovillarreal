...
export const AdminBookingDetailPage = () => {
  ...
  const [uploadBucket, setUploadBucket] = useState<BucketType>('output')
  const [galleryBucket, setGalleryBucket] = useState<BucketType>('output')
  ...
  const bucketMetrics = useMemo(
    () =>
      hasBooking && bookingId
        ...
        : { capture: 0, output: 0, selects: 0, trash: 0 },
  ...
  const outputFotos = useMemo(
    () => (hasBooking && bookingId ? getFotosByBooking(bookingId, 'output') : []),
    [hasBooking, bookingId, getFotosByBooking],
  )

  const aiTagsAvailable = useMemo(() => {
    const tags = new Set<string>()
    outputFotos.forEach((foto) => {
  ...
  const filteredFotos = useMemo(() => {
    if (galleryBucket !== 'output' || aiFilters.length === 0) return fotosBucket
  ...
  const aiSummary = useMemo(() => {
    if (!outputFotos.length) {
      return null
    }
    const total = outputFotos.length
    const avgQuality =
      outputFotos.reduce((acc, foto) => acc + (foto.ai_insights?.quality_score ?? 0), 0) / total
    const avgComposition =
      outputFotos.reduce((acc, foto) => acc + (foto.ai_insights?.composition_score ?? 0), 0) / total
    const closedEyes = outputFotos.filter((foto) => foto.ai_insights?.has_closed_eyes).length
    const blur = outputFotos.filter((foto) => foto.ai_insights?.has_blur).length
    const overExposure = outputFotos.filter((foto) => foto.ai_insights?.has_overexposure).length
    const groupPhotos = outputFotos.filter((foto) => foto.ai_insights?.is_group_photo).length
    return {
      total,
    ...
    <div className="space-y-10">
      <div className="flex flex-col gap-6 rounded-3xl border border-slate-200 bg-white p-8 shadow-subtle">
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            <button
              ...
              className="inline-flex items-center gap-2 rounded-full border border-slate-200 px-4 py-2 text-[10px] uppercase tracking-[0.14em] text-slate-500 transition hover:border-slate-900 hover:text-slate-900"
            >
              <LuArrowLeft /> Volver
            </button>
            <p className="mt-4 text-xs uppercase tracking-[0.22em] text-slate-400">Booking</p>
        ...
        <div className="grid gap-4 md:grid-cols-4">
          {(['output', 'capture', 'selects', 'trash'] as BucketType[]).map((bucket) => (
            <div
              key={bucket}
              className={`rounded-3xl border-2 bg-white p-5 shadow-sm ${bucketDetails[bucket].color} ${bucket === 'output' ? 'bg-emerald-50/50' : 'border-slate-200'}`}
            >
              ...
              <p className="mt-4 text-3xl font-semibold tracking-[0.14em] text-slate-900">
      ...
      <Card>
          ...
          <div className="mt-8 grid gap-8 lg:grid-cols-[1.1fr,0.9fr]">
            <div className="space-y-6">
                      ...
                      <div>
                        <p className="text-xs font-semibold uppercase tracking-[0.14em] text-slate-800">
                          ...
                          {bucketDetails[bucket].badge ? (
                            <span className="ml-2 rounded-full bg-emerald-500 px-2 py-1 text-[10px] uppercase tracking-[0.14em] text-white">
                              {bucketDetails[bucket].badge}
                            </span>
                          ) : null}
                        </p>
                        ...
                        {bucket === 'output' ? (
                          <p className="mt-2 text-[10px] uppercase tracking-[0.16em] text-emerald-500">
              ...
              >
                ...
                {uploadBucket === 'output' ? (
                  <p className="mt-3 text-[10px] uppercase tracking-[0.16em] text-emerald-500">
                    Aplicaremos AI Tagging inteligente al finalizar la subida
                  </p>
                ) : null}
                {processingUpload ? (
                  <div className="mt-6 inline-flex items-center gap-2 rounded-full bg-slate-900 px-4 py-2 text-[10px] uppercase tracking-[0.14em] text-white">
            ...
            <div className="rounded-3xl border border-slate-200 bg-white p-6">
              ...
              <div className="mt-6 space-y-4">
                {uploadQueue.length === 0 ? (
                  <div className="rounded-2xl border border-slate-200 bg-slate-50 p-6 text-[10px] uppercase tracking-[0.14em] text-slate-400">
                ...
                {uploadQueue.map((item) => (
                  <div
                    key={item.id}
                    className="rounded-2xl border border-slate-200 bg-slate-50 p-4 text-xs uppercase tracking-[0.16em] text-slate-500"
                  >
                    <div className="flex items-center justify-between">
        ...
        {activeTab === 'gallery' ? (
          <div className="mt-8 space-y-6">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
              <div className="flex flex-wrap gap-3">
                {(['output', 'capture', 'selects', 'trash'] as BucketType[]).map((bucket) => (
                  <button
                    ...
                    className={`rounded-full px-5 py-3 text-[10px] font-semibold uppercase tracking-[0.14em] transition ${galleryBucket === bucket ? 'bg-slate-900 text-white' : 'bg-slate-100 text-slate-500'}`}
                  >
                    {bucketDetails[bucket].title}
                  </button>
                ))}
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className={`rounded-full border px-4 py-2 text-[10px] uppercase tracking-[0.16em] ${viewMode === 'grid' ? 'border-slate-900 bg-slate-900 text-white' : 'border-slate-200 text-slate-500'}`}
                >
                  Grid
                </button>
                <button
                  onClick={() => setViewMode('list')}
                  className={`rounded-full border px-4 py-2 text-[10px] uppercase tracking-[0.16em] ${viewMode === 'list' ? 'border-slate-900 bg-slate-900 text-white' : 'border-slate-200 text-slate-500'}`}
                >
                  Lista
                </button>
              </div>
                ...
                <div className="mt-4 flex flex-wrap gap-2">
                  {aiTagsAvailable.map((tag) => (
                    <button
                      ...
                      className={`rounded-full px-4 py-2 text-[10px] uppercase tracking-[0.14em] transition ${aiFilters.includes(tag) ? 'bg-emerald-500 text-white' : 'bg-slate-100 text-slate-500 hover:bg-slate-200'}`}
                    >
                      #{tag}
                    </button>
                  ))}
            ...
            {viewMode === 'grid' ? (
                  ...
                  >
                    <div className="relative h-52 overflow-hidden">
                      ...
                      <div className="absolute left-3 top-3">
                        <Badge tone={foto.bucket_tipo === 'output' ? 'success' : 'muted'}>
                          {foto.bucket_tipo}
                        </Badge>
                      </div>
                      {foto.ai_insights?.has_closed_eyes || foto.ai_insights?.has_blur ? (
                        <div className="absolute bottom-3 left-3 rounded-full bg-rose-500 px-3 py-1 text-[10px] uppercase tracking-[0.14em] text-white">
                          {foto.ai_insights?.has_closed_eyes ? 'Ojos cerrados' : 'Revisar'}
                        </div>
                      ) : null}
                    </div>
                    <div className="space-y-3 px-4 py-3 text-[10px] uppercase tracking-[0.14em] text-slate-500">
              ...
              <div className="overflow-hidden rounded-3xl border border-slate-200">
                <table className="min-w-full divide-y divide-slate-100">
                  ...
                  <tbody className="divide-y divide-slate-100 bg-white text-xs uppercase tracking-[0.16em] text-slate-600">
                    {filteredFotos.map((foto) => (
                      <tr key={foto.id} className="hover:bg-slate-50">
                        <td className="px-4 py-4">
                          <div className="flex items-center gap-3">
                            ...
                            />
                            <div>
                              <p className="font-semibold tracking-[0.14em] text-slate-800">
                                {foto.nombre_archivo}
                              </p>
            ...
            {filteredFotos.length === 0 ? (
              <div className="rounded-3xl border border-slate-200 bg-slate-50 p-10 text-center text-xs uppercase tracking-[0.14em] text-slate-400">
        ...
        {activeTab === 'insights' ? (
          <div className="mt-8 space-y-6">
            {aiSummary ? (
              <>
                <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
                  <div className="rounded-3xl border border-slate-200 bg-white p-6">
                    ...
                    <p className="mt-3 text-3xl font-semibold tracking-[0.14em] text-slate-900">
                      {aiSummary.total}
                    </p>
                  </div>
                  <div className="rounded-3xl border border-slate-200 bg-white p-6">
                    ...
                    <p className="mt-3 text-3xl font-semibold tracking-[0.14em] text-emerald-600">
                ...
                <div className="grid gap-6 md:grid-cols-2">
                  <div className="rounded-3xl border border-slate-200 bg-emerald-50 p-6">
                    ...
                    <ul className="mt-4 space-y-2 text-xs uppercase tracking-[0.16em] text-emerald-700">
                  ...
                  <div className="rounded-3xl border border-slate-200 bg-slate-50 p-6">
                    ...
                    <ul className="mt-4 space-y-2 text-xs uppercase tracking-[0.16em] text-slate-600">
              ...
              <div className="rounded-3xl border border-slate-200 bg-slate-50 p-10 text-center text-xs uppercase tracking-[0.14em] text-slate-400">
                Este booking aún no tiene fotos en OUTPUT analizadas por IA.
              </div>
            )}
          </div>
        ) : null}
        ...
        {activeTab === 'info' ? (
          <div className="mt-8 grid gap-6 lg:grid-cols-2">
            <div className="rounded-3xl border border-slate-200 bg-slate-50 p-6 text-xs uppercase tracking-[0.16em] text-slate-500">
              <p className="text-slate-400">Resumen de sesión</p>
              <div className="mt-4 space-y-3 text-slate-600">
                <p>Creada: {new Date(booking.fecha_creacion).toLocaleDateString()}</p>
                <p>Entrega: {booking.fecha_entrega ? new Date(booking.fecha_entrega).toLocaleDateString() : 'Pendiente'}</p>
            ...
            <div className="rounded-3xl border border-slate-200 bg-white p-6">
              ...
              <textarea
                className="mt-4 h-40 w-full rounded-3xl border border-slate-200 bg-slate-50 p-5 text-xs uppercase tracking-[0.16em] text-slate-600 outline-none transition focus:border-slate-900"
